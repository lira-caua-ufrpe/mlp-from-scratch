...
class GraphVisualizer:
    ...
    def draw(self, epoch: int = 0, loss: float = 0.0, 
             show_weights: bool = True, show_gradients: bool = False,
        ...
        nx.draw_networkx_nodes(self.G, self.pos, node_color=node_colors, 
                              node_size=node_sizes, ax=self.ax, alpha=0.9)
        ...
        for u, v in self.G.edges():
            ...
            if show_gradients:
                ...
                edge_colors.append((1, 1 - intensity, 1 - intensity) if g >= 0 else (1 - intensity, 1 - intensity, 1))
            else:
                intensity = min(abs(w) * 2, 1.0)
                edge_colors.append((0.2, 0.2, 0.8, intensity) if w >= 0 else (0.8, 0.2, 0.2, intensity))
            edge_widths.append(max(abs(w) * 3, 0.5))
        
        if edge_weights:
            nx.draw_networkx_edges(self.G, self.pos, edgelist=edge_weights,
                                  edge_color=edge_colors, width=edge_widths,
                                  ax=self.ax, alpha=0.7, arrows=True, arrowsize=15)
        ...
        nx.draw_networkx_labels(self.G, self.pos, labels=node_labels, 
                               font_size=9, font_weight="bold", ax=self.ax)
        ...
        for layer_idx, nodes in layer_info.items():
            ...
            label = "Input" if layer_idx == 0 else ("Output" if layer_idx == max(layer_info.keys()) else f"Hidden {layer_idx}")
            self.ax.text(x, y_max, label, ha="center", va="bottom", 
                        fontsize=12, fontweight="bold", color="gray")
        
        self.ax.set_title(f"MLP Computational Graph | Epoch: {epoch} | Loss: {loss:.4f}", 
...
class TrainingVisualizer:
    ...
    def draw(self):
        ...
        self.axes[0].plot(epochs, self.history["train_loss"], "b-", label="Train Loss", linewidth=2)
        if self.history["val_loss"]:
            self.axes[0].plot(epochs[:len(self.history["val_loss"])], self.history["val_loss"], "r-", label="Val Loss", linewidth=2)
        ...
        self.axes[1].plot(epochs, self.history["train_acc"], "b-", label="Train Acc", linewidth=2)
        if self.history["val_acc"]:
            self.axes[1].plot(epochs[:len(self.history["val_acc"])], self.history["val_acc"], "r-", label="Val Acc", linewidth=2)
...
class WeightHistoryVisualizer:
    ...
    def draw(self):
        ...
        for snap in self.snapshots:
            self.ax.hist(snap["weights"], bins=30, alpha=0.3, 
                        label=f"Epoch {snap['epoch']}", density=True)
...
class LiveTrainingDashboard:
    ...
    def on_epoch_end(self, epoch: int, metrics: Dict, weights_snapshot: Dict):
        ...
        self.metrics_viz.update(
            ...
            metrics.get("val_loss"), metrics.get("val_acc")
        )
    ...
    def create_animation(self, output_path: str = "outputs/training_animation.gif", fps: int = 5):
