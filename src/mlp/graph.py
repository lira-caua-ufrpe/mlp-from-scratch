...
class ComputationalGraph:
    ...
    def connect_layers(
        self, source_layer_idx: int, target_layer_idx: int, weight_init: str = "xavier"
    ) -> List[Edge]:
    ...
    def backward_pass(self, targets: np.ndarray, loss_fn: str = "mse") -> float:
        ...
        if len(targets) != len(self.output_nodes):
            raise ValueError(
                f"Expected {len(self.output_nodes)} targets, got {len(targets)}"
            )
        ...
        for i, node in enumerate(self.output_nodes):
            if loss_fn == "mse":
                ...
                loss += -(
                    targets[i] * np.log(y_pred) + (1 - targets[i]) * np.log(1 - y_pred)
                )
    ...
    def get_topology(self) -> Dict:
        ...
        return {
            "layers": {
                idx: [n.node_id for n in nodes] for idx, nodes in self.layers.items()
            },
            "edges": [
                (e.source.node_id, e.target.node_id) for e in self.edges.values()
            ],
