...
from mlp.graph import ComputationalGraph
from data.loader import load_heart_disease, preprocess_data, get_data_info
from visualization.dashboard import GraphVisualizer, TrainingVisualizer
...
batch_size = st.sidebar.select_slider(
    "Batch Size", [8, 16, 32, 64, 128, 256], value=32
)
test_size = st.sidebar.slider("Test Size (%)", 10, 40, 20) / 100
val_split = st.sidebar.slider("Validation Split (%)", 5, 30, 15) / 100
weight_init = st.sidebar.selectbox(
    "Inicialização", ["xavier", "he", "random"], index=0
)
loss_fn = st.sidebar.selectbox(
    "Loss Function", ["bce", "mse"], index=0
)
...
# Main header
st.markdown('<div class="main-header">🧠 MLP from Scratch - Treino Interativo</div>', unsafe_allow_html=True)
...
with tab_data:
            ...
            with st.spinner("Carregando dados..."):
                try:
                    if dataset_option == "Heart Disease (UCI)":
                        ...
                        st.success(f"Heart Disease carregado: {X.shape[0]} amostras, {X.shape[1]} features")
                    else:
                        np.random.seed(seed)
                        X = np.random.randn(1000, input_size)
                        y = (X[:, 0] + X[:, 1] > 0).astype(float)
                        st.success(f"Dados sintéticos gerados: {X.shape[0]} amostras, {X.shape[1]} features")
    ...
    if st.session_state.training_data:
        ...
        dist_df = pd.DataFrame(list(info["class_distribution"].items()), columns=["Classe", "Contagem"])
...
with tab_architecture:
    ...
    with col2:
        ...
        if st.button("🎨 Visualizar Arquitetura"):
            dummy_mlp = MLP(layer_sizes, all_activations, weight_init=weight_init, seed=seed)
...
with tab_train:
    ...
    if st.button("▶️ Iniciar Treino", type="primary", disabled=train_disabled, use_container_width=True):
        ...
        def streamlit_callback(epoch, metrics, weights_snapshot):
            ...
            loss_df = pd.DataFrame({
                ...
                "Val Loss": hist["val_loss"] if hist["val_loss"] else [0]*len(hist["epochs"])
            })
            acc_df = pd.DataFrame({
                "Época": hist["epochs"],
                "Train Acc": hist["train_acc"],
                "Val Acc": hist["val_acc"] if hist["val_acc"] else [0]*len(hist["epochs"])
            })
            ...
            status_text.text(f"Época {epoch + 1}/{epochs} - Loss: {metrics['train_loss']:.4f}")
...
with tab_results:
    ...
    if st.session_state.mlp_model is None:
        ...
        col3.metric("Épocas Treinadas", len(st.session_state.training_history["epochs"]))
        ...
        if hist["epochs"]:
            ...
            with col1:
                loss_df = pd.DataFrame({
                    ...
                    "Val": hist["val_loss"] if hist["val_loss"] else [None]*len(hist["epochs"])
                })
            ...
            with col2:
                acc_df = pd.DataFrame({
                    ...
                    "Val": hist["val_acc"] if hist["val_acc"] else [None]*len(hist["epochs"])
                })
...
with tab_inference:
    ...
    if st.session_state.mlp_model is None:
        ...
        st.write("Insira valores para as features (serão normalizados automaticamente):")
        ...
        for i in range(input_size):
            with cols[i % 4]:
                val = st.number_input(f"Feature {i}", value=0.0, step=0.1, format="%.2f")
                inputs.append(val)
        ...
        if st.button("🔮 Prever", type="primary"):
            ...
            with col2:
                st.metric("Probabilidade", f"{proba[0] if isinstance(proba, np.ndarray) else proba:.2%}")
...
# Footer
st.divider()
st.caption("MLP from Scratch - Advanced AI Topics | Built with Streamlit, NumPy, Matplotlib")
