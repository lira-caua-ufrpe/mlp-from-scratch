 ...
 * Baseado em estágios comportamentais (Rumble, Escalation, Overload, Meltdown/Shutdown)
 * Ponte Multidisciplinar: Professor, Escola e Psicólogo/Terapeuta ABA
 */
...
class StimmingDetector {
    constructor() {
        ...
        this.poseHistory = [];
        this.faceHistory = [];
        this.timelineHistory = []; // Registro contínuo de estresse a cada segundo
        ...
        // Histórico de Eventos Clínicos para o Relatório do Psicólogo
        ...
        // Perfil de Sensibilidade configurável pelo professor/psicólogo
        this.profile = {
            auditorySensitivity: 1.0, // Multiplicador de sensibilidade
    ...
    /**
     * Processa o frame com análise em cascata
     */
    processFrame(poseLandmarks, faceLandmarks) {
        ...
        const auditoryDefense = this.detectAuditoryDefense(poseLandmarks, shoulderWidth, timestamp);
        ...
        // 4. Atualiza a timeline contínua (para o gráfico do psicólogo)
    ...
    /**
     * 1. Hand Flapping & Finger Motion (Estereotipia de Mãos)
     */
    ...
    /**
     * 2. Body Rocking (Balanço de Tronco / Autorregulação Vestibular)
     */
    ...
    /**
     * 3. Defesa Auditiva (Mãos nos Ouvidos / Hiperacusia)
     */
    detectAuditoryDefense(landmarks, scale, timestamp) {
        const lw = landmarks[15], rw = landmarks[16];
        const le = landmarks[7], re = landmarks[8];
        if (!lw || !rw || !le || !re) return { score: 0, detected: false, durationMs: 0 };
        ...
        if (isCovering) {
            ...
            const score = Math.min(100, Math.round(55 + Math.min(45, durationMs / 15) * this.profile.auditorySensitivity));
            return {
                score,
                detected: score >= 65,
    ...
    /**
     * 4. Defesa Visual / Esconder o Rosto (Fotofobia ou Sobrecarga Emocional)
     */
    detectVisualDefense(poseLandmarks, faceLandmarks, scale, timestamp) {
        ...
        // Ambas as mãos na frente dos olhos/nariz
    ...
    /**
     * 5. Head Nodding / Head Rolling (Movimento Rítmico da Cabeça)
     */
    ...
    /**
     * 6. Freeze / Shutdown (Imobilidade Súbita / Resposta de Congelamento)
     */
    detectFreezeShutdown(poseLandmarks, scale, timestamp) {
        ...
        const isRigid = avgMotion < 0.003; // Movimento quase nulo mantido

        if (isRigid) {
            if (!this.freezeStartTime) this.freezeStartTime = timestamp;
            const duration = timestamp - this.freezeStartTime;
            if (duration > 3000) { // Congelado por mais de 3 segundos
                return {
    ...
    /**
     * 7. Atenção e Contato Visual (Gaze & Attention via FaceMesh)
     */
    detectGazeAndAttention(faceLandmarks) {
        ...
        // Centro do nariz (1), Têmpora Esquerda (234), Têmpora Direita (454)
    ...
    /**
     * Síntese Clínica de 4 Fases (Rumble, Escalation, Overload, Meltdown/Shutdown)
     * e Recomendações Pedagógicas para o Professor
     */
    synthesizeClinicalCascade(data) {
        const { flapping, rocking, auditoryDefense, visualDefense, headNodding, freezeShutdown, gazeAttention, timestamp } = data;

        let stage = "CALM"; // CALM, RUMBLE, ESCALATION, SENSORY_OVERLOAD, SHUTDOWN
        let stageName = "🟢 Estágio 1: Calmo & Regulado";
        let title = "Comportamento Estável";
        let desc = "Aluno com níveis basais de autorregulação e atenção.";
        let severity = "normal"; // normal, warning, critical, alert
        ...
        if (auditoryDefense.detected) {
            ...
            stageName = "🔴 Estágio 4: Sobrecarga Sensorial Crítica (Acústica)";
            title = "⚠️ Defesa Sensorial Auditiva";
            desc = `Aluno cobrindo os ouvidos (${auditoryDefense.type}) há ${auditoryDefense.durationMs}ms.`;
            severity = "critical";
            recommendation = "🚨 AÇÃO IMEDIATA: Oferecer fone abafador de ruído, reduzir tom de voz ou permitir ida ao cantinho sensorial.";
    ...
    getDefaultResult() {
        return {
            ...
            desc: 'Posicione o dispositivo para capturar tronco e rosto do aluno.',
            severity: 'normal',
